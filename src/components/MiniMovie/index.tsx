...
const MiniMovie = ({ movie, classNameImg, classNameContainer }: Props) => {
  moment.locale("pt-BR");
  return (
    <Link to={`/movie/${movie.id}`} className="remove-link">
      <div className={classNameContainer + " container-mini-movie"}>
        <img
          className={classNameImg + " img-mini-movie"}
          alt={movie.title}
          src={
            movie.poster_path !== ""
        ...
        <div className="container-title">
          ...
          <label>{moment(new Date(movie.release_date)).format("DD MMM YYYY")}</label>
        </div>
      </div>
    </Link>
  );
